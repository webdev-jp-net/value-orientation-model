...
- **帰属志向（Y軸）**: `オーナーシップ（平均）` - `コンセンサス（平均）`
- **関係性志向（X軸）**: `融合（平均）` - `自立（平均）`

それぞれの変数は、各軸に割り当てられた設問回答の合算（各 -10〜10）を設問数で割った平均値（-2〜2）として算出される。
座標レンジ：オーナーシップ（4）←→ コンセンサス（-4）、融合（4）←→ 自立（-4）
※内部的にはこのレンジをマトリクスの座標系（±160px）にマッピングしている。
...
- **カンマ区切りによる一括入力**:
  `オーナーシップ`, `コンセンサス`, `自立`, `融合` の4つの数値（設問合計値）は、カンマ区切りによる一括テキスト入力（例: `10, 5, -5, 10`）により行います。
