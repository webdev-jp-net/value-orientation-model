...
#### ポイント算出方法
10問の設問回答（各 -2〜2）を合算し、帰属志向のポイントとしてY軸のプロット座標とする。
...
#### ポイント算出方法
10問の設問回答（各 -2〜2）を合算し、関係性志向のポイントとしてX軸のプロット座標とする。
自立（-20）←→ 融合（20）
...
`名前`, `帰属`, `関係性` の3項目を1セットのレコード（パーソナルプロットの構成要素）として扱います。
...
文章問題ではシチュエーションの提示があり、設問ごとに定義されたラベル（「否定」から「肯定」など）へのフィーリングを5段階の段階選択からの単一選択です。  
...
- **カンマ区切りによる一括入力**:
  `帰属`, `関係性` の2つの数値（各 -20〜20）は、カンマ区切りによる一括テキスト入力（例: `15, -10`）により行います。
