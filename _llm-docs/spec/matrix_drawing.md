...
価値志向モデルのマトリクス（分布図）を描画するための技術仕様および実装詳細。
...
- **技術スタック**:
  - **ライブラリ**: VisX（Airbnb製Reactビジュアライゼーションライブラリ）
    - 使用パッケージ: `@visx/group`, `@visx/shape`, `@visx/scale`, `@visx/responsive`, `@visx/marker`
  - **描画方式**: SVG（Scalable Vector Graphics）
- **データソース**: `data/questionList.json`（設問数に基づく平均値算出に使用）

## 描画ロジック

### 座標系
- **中心点**: マトリクスの幾何学的な中心を原点（0, 0）とする。
- **スケール**: 各指標の平均値（-2〜2）の差分に基づき、±4の範囲をマトリクスの表示領域にマッピングする。
- **反転**: Y軸（帰属志向）は、数学的な正負とSVGの座標系（上が0）を合わせるため、描画時に反転処理を行う。

### 意匠（デザイン要素）
- **十字矢印**: 中心から上下左右の端点に向かう軸線。先端には三角形の矢印（SVG Marker）を配置する。
